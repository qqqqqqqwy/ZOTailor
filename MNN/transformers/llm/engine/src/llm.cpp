...
static std::string debugVarShape(Express::VARP var) {
...
static std::string debugVarType(Express::VARP var) {
    if (var.get() == nullptr || var->getInfo() == nullptr) {
        return "<null>";
    }
    const auto type = var->getInfo()->type;
    std::ostringstream os;
...
template <typename T>
static void debugPrintTypedVarSummary(const char* label, int forwardDebugId, Express::VARP var) {
    const auto* info = var->getInfo();
    const T* data = var->readMap<T>();
    if (data == nullptr || info->size <= 0) {
        std::cerr << "llm_forward_debug_tensor"
                  << " id=" << forwardDebugId
                  << " label=" << label
                  << " shape=" << debugVarShape(var)
                  << " type=" << debugVarType(var)
                  << " size=" << (info != nullptr ? info->size : 0)
                  << " data=<null>\n";
        return;
    }

    const size_t total = static_cast<size_t>(info->size);
    const size_t statCount = std::min<size_t>(total, 1024);
    double sum = 0.0;
    double sumSq = 0.0;
    double maxAbs = 0.0;
    double minValue = static_cast<double>(data[0]);
    double maxValue = static_cast<double>(data[0]);
    ...
    std::cerr << "llm_forward_debug_tensor"
              ...
              << " size=" << total
              << " sampled=" << statCount
              << " mean=" << (sum / static_cast<double>(statCount))
              << " rms=" << std::sqrt(sumSq / static_cast<double>(statCount))
              << " min=" << minValue
              << " max=" << maxValue
              << " max_abs=" << maxAbs
              << " first=[";
    const size_t valueCount = std::min<size_t>(total, 6);
    for (size_t i = 0; i < valueCount; ++i) {
        if (i > 0) {
            std::cerr << ",";
        }
        std::cerr << static_cast<double>(data[i]);
    }
    std::cerr << "]\n";
}

static void debugPrintVarSummary(const char* label, int forwardDebugId, Express::VARP var) {
    if (var.get() == nullptr || var->getInfo() == nullptr) {
        std::cerr << "llm_forward_debug_tensor"
                  << " id=" << forwardDebugId
                  << " label=" << label
                  << " shape=<null> type=<null> size=0\n";
        return;
    }
    const auto type = var->getInfo()->type;
    if (type.code == halide_type_float && type.bits == 32) {
        debugPrintTypedVarSummary<float>(label, forwardDebugId, var);
        return;
    }
    if (type.code == halide_type_float && type.bits == 16) {
        debugPrintTypedVarSummary<half_float::half>(label, forwardDebugId, var);
        return;
    }
    if (type.code == halide_type_int && type.bits == 32) {
        debugPrintTypedVarSummary<int>(label, forwardDebugId, var);
        return;
    }
    std::cerr << "llm_forward_debug_tensor"
              << " id=" << forwardDebugId
...
void Llm::setExtraInputNames(const std::vector<std::string>& names) {
    mExtraInputNames = names;
}

void Llm::setExtraInputs(const Express::VARPS& inputs) {
    mExtraInputs = inputs;
}

void Llm::setRuntimeHint(std::shared_ptr<Express::Executor::RuntimeManager> &rtg) {
    int initThreads = mConfig->thread_num();
    if (initThreads <= 0) {
        initThreads = 4;
    }
    rtg->setHint(MNN::Interpreter::INIT_THREAD_NUMBER, initThreads);
...
void Llm::initRuntime() {
    ...
    const bool mixedFp32Attention = mConfig->config_.value("mixed_fp32_attention", false);
    const bool mixedFp32AttentionContext = mConfig->config_.value("mixed_fp32_attention_context", mixedFp32Attention);
    if (mMeta) {
        mMeta->attention_fp32_score = mixedFp32Attention;
        mMeta->attention_fp32_context = mixedFp32Attention && mixedFp32AttentionContext;
    }
    if (mConfig->precision() == "high") {
        cpuBackendConfig.precision = BackendConfig::Precision_High;
    } else if (mConfig->precision() == "low" || (mConfig->precision() == "normal" && mixedFp32Attention)) {
...
bool Llm::load() {
    ...
    inputNames.insert(inputNames.end(), mExtraInputNames.begin(), mExtraInputNames.end());
...
std::vector<Express::VARP> Llm::forwardRaw(Express::VARP hiddenState, Express::VARP mask, Express::VARP inputPos, Express::VARPS extraArgs) {
    ...
    inputs.insert(inputs.end(), mExtraInputs.begin(), mExtraInputs.end());

    static std::atomic<int> sForwardDebugPrints{0};
    int forwardDebugId = -1;
    if (mConfig->config_.value("enable_debug", false)) {
        const int nextId = sForwardDebugPrints.fetch_add(1);
        if (nextId < 32) {
            forwardDebugId = nextId;
            std::cerr << "llm_forward_debug_before"
                      << " id=" << forwardDebugId
                      << " seq_len=" << seqLen
                      << " seq_len_key=" << seqLenKey
                      << " in_decode=" << (inDecode ? "true" : "false")
                      << " all_logits=" << (isAllLogists ? "true" : "false")
                      << " meta_add=" << mMeta->add
                      << " base_inputs=4"
                      << " extra_args=" << extraArgs.size()
                      << " extra_inputs=" << mExtraInputs.size()
                      << " extra_input_names=" << mExtraInputNames.size()
                      << " total_inputs=" << inputs.size()
                      << " hidden_shape=" << debugVarShape(hiddenState)
                      << " mask_shape=" << debugVarShape(mask)
                      << " pos_shape=" << debugVarShape(inputPos)
                      << " logits_index_shape=" << debugVarShape(logitsIndex)
                      << "\n";
            debugPrintVarSummary("hidden", forwardDebugId, hiddenState);
            debugPrintVarSummary("mask", forwardDebugId, mask);
            debugPrintVarSummary("position_ids", forwardDebugId, inputPos);
            debugPrintVarSummary("logits_index", forwardDebugId, logitsIndex);
            for (size_t i = 0; i < extraArgs.size(); ++i) {
                if (i >= 4 && i + 1 != extraArgs.size()) {
                    continue;
                }
                const std::string label = "extra_arg_" + std::to_string(i);
                debugPrintVarSummary(label.c_str(), forwardDebugId, extraArgs[i]);
            }
            for (size_t i = 0; i < mExtraInputs.size(); ++i) {
                if (i >= 6 && i + 1 != mExtraInputs.size()) {
                    continue;
                }
                std::cerr << "llm_forward_debug_extra_input"
                          << " id=" << forwardDebugId
                          << " index=" << i
                          << " name=\"" << (i < mExtraInputNames.size() ? mExtraInputNames[i] : std::string())
                          << "\" shape=" << debugVarShape(mExtraInputs[i])
                          << " type=" << debugVarType(mExtraInputs[i])
                          << "\n";
                const std::string label = "extra_input_" + std::to_string(i);
                debugPrintVarSummary(label.c_str(), forwardDebugId, mExtraInputs[i]);
            }
        }
    }
    ...
    if (outputs.empty()) {
        if (forwardDebugId >= 0) {
            std::cerr << "llm_forward_debug_after"
                      << " id=" << forwardDebugId
                      << " outputs=0 status=empty\n";
        }
    ...
    if (forwardDebugId >= 0) {
        std::cerr << "llm_forward_debug_after"
                  << " id=" << forwardDebugId
                  << " outputs=" << outputs.size()
                  << " output0_shape=" << debugVarShape(outputs[0])
                  << " valid_start=" << mGenerateParam->validLogitStart
                  << " valid_size=" << mGenerateParam->validLogitSize
                  << "\n";
        debugPrintVarSummary("output0", forwardDebugId, outputs[0]);
    }
...
int Llm::getValidLogitStart() const {
    return mGenerateParam ? mGenerateParam->validLogitStart : 0;
}

int Llm::getValidLogitSize() const {
    return mGenerateParam ? mGenerateParam->validLogitSize : 0;
}
...
VARP Llm::gen_attention_mask(int seq_len) {
    ...
    if (mConfig->attention_mask() == "float") {
        ...
        // Mask: lower triangular
        const bool forceFullCausalMask = mConfig->config_.value("force_full_causal_mask", false);
        if (mConfig->backend_type() == "cpu" && mValidBlockSize.empty() && !forceFullCausalMask) { // Now only cpu supports using lower triangular to opt the attention performance
