...
class LlmExporter(torch.nn.Module):
    ...
    def export_config(self, mnn_config = False):
        ...
        with open(f'{self.args.dst_path}/config.json', 'w', encoding='utf-8') as f:
            ...
            tokenizer_mtok = os.path.join(self.args.dst_path, 'tokenizer.mtok')
            tokenizer_txt = os.path.join(self.args.dst_path, 'tokenizer.txt')
            config['tokenizer_file'] = 'tokenizer.mtok' if os.path.exists(tokenizer_mtok) or not os.path.exists(tokenizer_txt) else 'tokenizer.txt'
    ...
    def export_tokenizer(self):
        return self.tokenizer.export(
            self.args.dst_path,
            model_path=self.args.tokenizer_path or self.args.path,
