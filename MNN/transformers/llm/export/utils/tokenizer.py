...
class LlmTokenizer(PreTrainedTokenizer):
    ...
    def export(self, save_directory, model_path=None, model_type=None):
        ...
        def _existing_path(path, base=None):
            if not path:
                return None
            path = os.fspath(path)
            candidates = [path]
            if base is not None and not os.path.isabs(path):
                candidates.insert(0, os.path.join(base, path))
            for candidate in candidates:
                if os.path.exists(candidate):
                    return candidate
            return None

        def _cached_tokenizer_json(repo_or_path):
            if not repo_or_path:
                return None
            try:
                from transformers.utils import cached_file
                return cached_file(repo_or_path, 'tokenizer.json', local_files_only=True)
            except Exception:
                return None

        def _materialize_fast_tokenizer_json():
            try:
                ...
                return _existing_path(os.path.join(save_directory, 'tokenizer.json'))
            except Exception as e:
                print(f"Warning: failed to export fast tokenizer.json for .mtok: {e}")
                return None

        # Prefer .mtok because it preserves the tokenizer.json pipeline. The
        # text SentencePiece fallback is less faithful for LLaMA-family unigram
        # tokenizers.
        tokenizer_json_candidates = [
            _existing_path('tokenizer.json', model_path),
            _cached_tokenizer_json(model_path),
            _cached_tokenizer_json(self.tokenizer_path),
        ]
        init_kwargs = getattr(self.tokenizer, 'init_kwargs', None)
        if isinstance(init_kwargs, dict):
            tokenizer_json_candidates.extend([
                _existing_path(init_kwargs.get('tokenizer_file'), model_path),
                _existing_path(init_kwargs.get('tokenizer_file')),
            ])
        tokenizer_json_candidates.append(
            _existing_path(os.path.join(getattr(self.tokenizer, 'name_or_path', ''), 'tokenizer.json'))
        )

        tokenizer_json_path = next((p for p in tokenizer_json_candidates if p is not None), None)
        if tokenizer_json_path is None:
            # Only materialize via AutoTokenizer.save_pretrained as a last resort:
            # it may hit the network and writes extra files into save_directory.
            tokenizer_json_path = _materialize_fast_tokenizer_json()
        ...
        # Load SentencePiece model if available. The tokenizer path can be a
        # HuggingFace model id, so also consult the instantiated tokenizer for
        # the cached vocab_file instead of only checking model_path directly.
        sp_model = None
        sp_model_source = None

        try:
            import sentencepiece as spm
            candidates = [
                os.path.join(model_path, 'tokenizer.model'),
                os.path.join(model_path, 'ice_text.model'),
                getattr(self.tokenizer, 'vocab_file', None),
            ]
            init_kwargs = getattr(self.tokenizer, 'init_kwargs', None)
            if isinstance(init_kwargs, dict):
                candidates.extend([
                    init_kwargs.get('vocab_file'),
                    init_kwargs.get('tokenizer_file'),
                ])
            for candidate in candidates:
                candidate = _existing_path(candidate, model_path)
                if candidate is not None:
                    sp_model = spm.SentencePieceProcessor(candidate)
                    sp_model_source = candidate
                    break
            if sp_model is None:
                direct_sp_model = getattr(self.tokenizer, 'sp_model', None)
                if direct_sp_model is not None and hasattr(direct_sp_model, 'GetPieceSize'):
                    sp_model = direct_sp_model
                    sp_model_source = 'tokenizer.sp_model'
        except Exception as e:
            print(f"Warning: failed to load SentencePiece model: {e}")
            sp_model = None
        ...
        if sp_model is not None:
            if sp_model_source is not None:
                print(f"Exporting SentencePiece tokenizer from: {sp_model_source}")
            ...
            for i in range(sp_model.GetPieceSize()):
                ...
                # Keep the SentencePiece word-start marker. The C++ runtime
                # normalizes spaces to U+2581 before lookup, so replacing it
                # here with an ASCII space makes normal word pieces unreachable.
