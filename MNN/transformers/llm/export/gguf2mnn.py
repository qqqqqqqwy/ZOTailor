...
def call_mnnconvert(args, mnnconvert=None):
    if mnnconvert:
...
def ensure_mnn_json(mnn_dir, mnnconvert=None):
    src_json = os.path.join(mnn_dir, "llm.mnn.json")
    if os.path.exists(src_json):
        return src_json
    input_mnn = os.path.join(mnn_dir, "llm.mnn")
    if not os.path.exists(input_mnn):
        raise FileNotFoundError(
            f"missing MNN graph: {input_mnn}\n"
            "gguf2mnn.py only replaces weights in an existing MNN graph skeleton. "
            "Run transformers/llm/export/llmexport.py first with --quant_bit 16 "
            "and use that output directory as --mnn_dir."
        )
    print("Dump MNN graph to JSON:", src_json)
    call_mnnconvert(["", "-f", "MNN", "--modelFile", input_mnn, "--JsonFile", src_json], mnnconvert)
    if not os.path.exists(src_json):
        raise RuntimeError(f"failed to generate {src_json}")
    return src_json
...
def print_match_summary(opmap, convs, tensormap):
    conv_missing = []
    conv_matches = []
    conv_type_counts = defaultdict(int)
    norm_missing = []
    norm_matches = 0
    norm_type_counts = defaultdict(int)

    for name, op in opmap.items():
        if op.get('type') != 'LayerNorm':
            continue
        weight_name = name + '.weight'
        if weight_name not in tensormap:
            norm_missing.append(weight_name)
            continue
        norm_matches += 1
        norm_type_counts[tensor_type_name(tensormap[weight_name].tensor_type)] += 1
    ...
    for op in convs:
        ...
        if weight_name not in tensormap:
            if name == 'output' and 'token_embd.weight' in tensormap:
                source_name = 'token_embd.weight'
            else:
                conv_missing.append(weight_name)
                continue
        weight = tensormap[source_name]
        weight_type = tensor_type_name(weight.tensor_type)
        conv_type_counts[weight_type] += 1
        conv_matches.append((name, source_name, weight_type, list(weight.shape)))

    print("\n=== GGUF -> MNN weight match summary ===")
    print(f"LayerNorm matched {norm_matches}/{norm_matches + len(norm_missing)}")
    for key in sorted(norm_type_counts):
        print(f"  LayerNorm type {key}: {norm_type_counts[key]}")
    if norm_missing:
        print("  First missing LayerNorm weights:")
        for name in norm_missing[:10]:
            print("   ", name)

    print(f"Convolution/Linear matched {len(conv_matches)}/{len(convs)}")
    for key in sorted(conv_type_counts):
        print(f"  Linear type {key}: {conv_type_counts[key]}")
    print("  First matched Linear weights:")
    for name, source_name, weight_type, shape in conv_matches[:10]:
        print(f"    {name}.weight <- {source_name} type={weight_type} shape={shape}")
    if conv_missing:
        print("  First missing Linear weights:")
        for name in conv_missing[:10]:
            print("   ", name)
    print("=== end match summary ===\n")
    return conv_missing, norm_missing
...
def convert(args):
    ...
    src_json = ensure_mnn_json(mnn_dir, args.mnnconvert)
    ...
    conv_missing, norm_missing = print_match_summary(opmap, convs, tensormap)
    if conv_missing or norm_missing:
        raise RuntimeError("GGUF tensor names do not fully match the MNN graph; see match summary above")
    if args.dry_run_match:
        return
    ...
    print(convert_args)
    call_mnnconvert(convert_args, args.mnnconvert)
    os.replace(dst_json, src_json)
    print("Update MNN graph JSON:", src_json)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dry-run-match', action='store_true', help='Only print GGUF/MNN tensor match summary and exit')
