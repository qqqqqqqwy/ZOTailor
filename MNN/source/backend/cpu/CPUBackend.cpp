...
Backend* CPURuntime::onCreate(const BackendConfig* config, Backend* origin) const {
    ...
    if (std::getenv("MNN_ZO_DEBUG_BACKEND") != nullptr) {
        MNN_PRINT("MNN_ZO_DEBUG_BACKEND precision=%d memory=%d flags=%zu backend_type=%d core_bytes=%d matmul_bytes=%d threads=%d init_threads=%d\n",
                  static_cast<int>(precision),
                  static_cast<int>(memory),
                  flags,
                  static_cast<int>(res->type()),
                  res->functions()->bytes,
                  res->functions()->matmulBytes,
                  res->threadNumber(),
                  initThreadNumber);
    }
