...
Execution* ConvolutionFloatFactory::create(const std::vector<Tensor*>& inputs, const std::vector<Tensor*>& outputs,
    ...
    if (nullptr != conv2d->quanParameter()) {
        bool forceFloat = static_cast<CPUBackend*>(backend)->getRuntime()->hint().forceFloatWeight;
        ...
        const bool forceQuant = forceFloat ? false : lowMemory;
        quanCommon = ConvolutionCommon::load(op, backend, forceFloat, forceQuant);
