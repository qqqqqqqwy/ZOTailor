...
run_one() {
  ...
  set +e
  B=zoo-cpu ./scripts/snapdragon/adb/run-tool.sh "$TARGET_BIN" \
    --mode cpu --pipeline false --antithetic false --device none \
    --lora "$lora" \
    -m "$model" \
    -fa on --eval-step -1 --steps 5 2>&1 | tee "$run_log"
...
run_one tiny   /data/local/tmp/gguf/Tiny-lora.gguf   /data/local/tmp/gguf/TinyLlama-1.1B-Chat-v1.0-Q4_0.gguf
echo "Sleeping ${SLEEP_BETWEEN}s before next run..."
sleep "$SLEEP_BETWEEN"

run_one llama  /data/local/tmp/gguf/Llama-lora.gguf  /data/local/tmp/gguf/Llama-3.2-3B-Q4_0.gguf
echo "Sleeping ${SLEEP_BETWEEN}s before next run..."
sleep "$SLEEP_BETWEEN"

run_one gemma  /data/local/tmp/gguf/gemma-lora.gguf  /data/local/tmp/gguf/gemma-3-4b-it-Q4_0.gguf
