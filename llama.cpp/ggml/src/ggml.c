...
struct ggml_tensor * ggml_lora_mul_mat(
        struct ggml_context * ctx,
        struct ggml_tensor  * a,
        struct ggml_tensor  * b,
        ...
        float                 scale) {
    GGML_ASSERT(ggml_can_mul_mat(a, b));
    GGML_ASSERT(ggml_can_mul_mat(lora_a, b));
    GGML_ASSERT(!ggml_is_transposed(a));
    GGML_ASSERT(!ggml_is_transposed(lora_a));
    GGML_ASSERT(!ggml_is_transposed(lora_b));
    GGML_ASSERT(lora_a->ne[0] == a->ne[0]);
    GGML_ASSERT(lora_b->ne[0] == lora_a->ne[1]);
    GGML_ASSERT(lora_b->ne[1] == a->ne[1]);

    const int64_t ne[4] = { a->ne[1], b->ne[1], b->ne[2], b->ne[3] };
    struct ggml_tensor * result = ggml_new_tensor(ctx, GGML_TYPE_F32, 4, ne);

    result->op     = GGML_OP_LORA_MUL_MAT;
    result->src[0] = a;
    result->src[1] = b;
    result->src[2] = lora_a;
    result->src[3] = lora_b;
...
struct ggml_tensor * ggml_lora_mul_mat_antithetic(
        struct ggml_context * ctx,
        struct ggml_tensor  * a,
        struct ggml_tensor  * b,
        struct ggml_tensor  * lora_a,
        struct ggml_tensor  * lora_b_plus,
        struct ggml_tensor  * lora_b_minus,
        float                 scale,
    ...
    GGML_ASSERT(!ggml_is_transposed(a));
    GGML_ASSERT(!ggml_is_transposed(lora_a));
    GGML_ASSERT(!ggml_is_transposed(lora_b_plus));
    GGML_ASSERT(!ggml_is_transposed(lora_b_minus));
    GGML_ASSERT(lora_a->ne[0] == a->ne[0]);
    GGML_ASSERT(lora_b_plus->ne[0] == lora_a->ne[1]);
    GGML_ASSERT(lora_b_plus->ne[1] == a->ne[1]);
    GGML_ASSERT(lora_b_minus->type  == lora_b_plus->type);
    GGML_ASSERT(lora_b_minus->ne[0] == lora_b_plus->ne[0]);
    GGML_ASSERT(lora_b_minus->ne[1] == lora_b_plus->ne[1]);
    GGML_ASSERT(lora_b_minus->ne[2] == lora_b_plus->ne[2]);
    GGML_ASSERT(lora_b_minus->ne[3] == lora_b_plus->ne[3]);
    GGML_ASSERT(positive_rows > 0);
    ...
    result->src[1] = b;
    result->src[2] = lora_a;
    result->src[3] = lora_b_plus;
    result->src[4] = lora_b_minus;
...
struct ggml_tensor * ggml_lora_mul_mat_antithetic_rm(
        struct ggml_context * ctx,
        struct ggml_tensor  * a,
        struct ggml_tensor  * b,
        struct ggml_tensor  * lora_a,
        struct ggml_tensor  * lora_b_plus_rm,
        struct ggml_tensor  * lora_b_minus_rm,
        float                 scale,
        int32_t               positive_rows) {
    GGML_ASSERT(ggml_can_mul_mat(a, b));
    GGML_ASSERT(ggml_can_mul_mat(lora_a, b));
    GGML_ASSERT(!ggml_is_transposed(a));
    GGML_ASSERT(!ggml_is_transposed(lora_a));
    GGML_ASSERT(lora_b_plus_rm->type  == GGML_TYPE_F32);
    GGML_ASSERT(lora_b_minus_rm->type == GGML_TYPE_F32);
    GGML_ASSERT(lora_a->ne[0] == a->ne[0]);
    GGML_ASSERT(lora_b_plus_rm->ne[0] == a->ne[1]);
    GGML_ASSERT(lora_b_plus_rm->ne[1] == lora_a->ne[1]);
    GGML_ASSERT(lora_b_minus_rm->ne[0] == lora_b_plus_rm->ne[0]);
    GGML_ASSERT(lora_b_minus_rm->ne[1] == lora_b_plus_rm->ne[1]);
    GGML_ASSERT(lora_b_minus_rm->ne[2] == lora_b_plus_rm->ne[2]);
    GGML_ASSERT(lora_b_minus_rm->ne[3] == lora_b_plus_rm->ne[3]);
    GGML_ASSERT(positive_rows > 0);
    ...
    result->src[1] = b;
    result->src[2] = lora_a;
    result->src[3] = lora_b_plus_rm;
    result->src[4] = lora_b_minus_rm;
