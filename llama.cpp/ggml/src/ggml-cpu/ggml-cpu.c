...
static inline float ggml_cpu_get_f32_2d(const struct ggml_tensor * t, int64_t i0, int64_t i1) {
    const char * p = (const char *) t->data + i1*t->nb[1] + i0*t->nb[0];

    switch (t->type) {
        case GGML_TYPE_F32:
            return *(const float *) p;
        case GGML_TYPE_F16:
            return GGML_CPU_FP16_TO_FP32(*(const ggml_fp16_t *) p);
        default:
            GGML_ABORT("%s: unsupported tensor type: %s\n", __func__, ggml_type_name(t->type));
    }
}

static inline void ggml_cpu_set_f32_2d(struct ggml_tensor * t, int64_t i0, int64_t i1, float v) {
    char * p = (char *) t->data + i1*t->nb[1] + i0*t->nb[0];
...
static void ggml_compute_forward_lora_mul_mat(
        const struct ggml_compute_params * params,
              struct ggml_tensor * dst) {
    GGML_UNUSED(params);
    ...
    GGML_ASSERT(lora_a->type == GGML_TYPE_F16 || lora_a->type == GGML_TYPE_F32);
    GGML_ASSERT(lora_b->type == GGML_TYPE_F16 || lora_b->type == GGML_TYPE_F32);
    GGML_ASSERT(lora_b_minus == NULL || lora_b_minus->type == GGML_TYPE_F16 || lora_b_minus->type == GGML_TYPE_F32);
    GGML_ASSERT(dst->ne[2] == 1 && dst->ne[3] == 1);
    GGML_ASSERT(x->ne[2] == 1 && x->ne[3] == 1);

    float scale = 0.0f;
    memcpy(&scale, dst->op_params, sizeof(scale));
    int32_t positive_rows = 0;
    int32_t lora_flags = 0;
    if (lora_b_minus != NULL) {
        struct {
            float   scale;
        ...
        positive_rows = lora_params.positive_rows;
        lora_flags = lora_params.flags;
        GGML_ASSERT(positive_rows > 0 && positive_rows <= x->ne[1]);
        GGML_ASSERT(lora_b_minus->ne[0] == lora_b->ne[0]);
        GGML_ASSERT(lora_b_minus->ne[1] == lora_b->ne[1]);
    }
    const bool b_rm_f32 = (lora_flags & (1 << 1)) != 0;
    ...
    for (int64_t im = 0; im < m; ++im) {
        for (int64_t ir = 0; ir < r; ++ir) {
            ...
            for (int64_t ik = 0; ik < k; ++ik) {
                sum += ggml_cpu_get_f32_2d(lora_a, ik, ir) * ggml_cpu_get_f32_2d(x, ik, im);
            }
            tmp[(size_t) im * (size_t) r + (size_t) ir] = sum;
        }
    }
    ...
    for (int64_t im = 0; im < m; ++im) {
        for (int64_t in = 0; in < n; ++in) {
            float sum = 0.0f;
            for (int64_t ik = 0; ik < k; ++ik) {
                sum += ggml_cpu_get_f32_2d(w, ik, in) * ggml_cpu_get_f32_2d(x, ik, im);
            }

            float delta = 0.0f;
            const struct ggml_tensor * lora_b_cur =
                (lora_b_minus != NULL && im >= positive_rows) ? lora_b_minus : lora_b;
            for (int64_t ir = 0; ir < r; ++ir) {
                const float bv = b_rm_f32 ? ggml_cpu_get_f32_2d(lora_b_cur, in, ir) : ggml_cpu_get_f32_2d(lora_b_cur, ir, in);
                delta += bv * tmp[(size_t) im * (size_t) r + (size_t) ir];
            }

            ggml_cpu_set_f32_2d(dst, in, im, sum + scale * delta);
        }
    }
...
static void ggml_compute_forward(struct ggml_compute_params * params, struct ggml_tensor * tensor) {
    ...
    switch (tensor->op) {
        ...
        case GGML_OP_LORA_MUL_MAT:
            {
                ggml_compute_forward_lora_mul_mat(params, tensor);
            } break;
