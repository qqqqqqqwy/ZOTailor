    ...
    for (int b3 = 0; b3 < params->ne13 && ret == 0; ++b3) {
        for (int b2 = 0; b2 < params->ne12 && ret == 0; ++b2) {
            ret = hmx_mat_mul_permuted_w16a32(ctx,
                                              ...
                                              params->act_stride, params->weight_stride, params->weight_pre_tiled);
        }
    }
    return ret;
}

static inline const __fp16 * hmx_pre_tiled_weight_chunk(const __fp16 * weight, size_t nc, int k) {
    return weight + (nc / HMX_FP16_TILE_N_COLS) * (k / HMX_FP16_TILE_N_COLS) * HMX_FP16_TILE_N_ELMS;
}

static inline size_t hmx_pre_tiled_weight_chunk_bytes(size_t n_cols, int k) {
    return (n_cols / HMX_FP16_TILE_N_COLS) * (k / HMX_FP16_TILE_N_COLS) * HMX_FP16_TILE_N_ELMS * sizeof(__fp16);
}
...
int hmx_mat_mul_permuted_w16a32_batched(struct htp_context *ctx, const hmx_matmul_w16a32_batched_params_t *params) {
    ...
    if (params->act_stride < params->k || (!params->weight_pre_tiled && params->weight_stride < params->k) || params->dst_stride < params->n) { return -1; }
    ...
    // Pre-tiled weights skip per-matmul row-major scratch and interleave.
    const size_t weight_per_n = params->weight_pre_tiled ? vec_dot_size : 3 * vec_dot_size;
    ...
    const size_t scratch_area_size    = params->weight_pre_tiled ? 0 : hex_align_up(n_chunk_n_cols * vec_dot_size, HMX_FP16_TILE_SIZE);
    ...
    void    *vtcm_scratch0   = params->weight_pre_tiled ? NULL : vtcm_seq_alloc(&vtcm_ptr, scratch_area_size);
    void    *vtcm_scratch1   = params->weight_pre_tiled ? NULL : vtcm_seq_alloc(&vtcm_ptr, scratch_area_size);
    ...
    FARF(MEDIUM, "%s: grouped path m=%d k=%d n=%d group=%d streams=%d mc=%zu nc=%zu pretiled=%d vtcm=%zu/%zu",
            __func__, params->m, params->k, params->n, group_size, params->ne13,
            m_chunk_n_rows, n_chunk_n_cols, params->weight_pre_tiled,
        ...
        for (int b2_base = 0; b2_base < params->ne12; b2_base += group_size) {
            ...
            for (size_t mr = 0; mr < (size_t) params->m; mr += m_chunk_n_rows) {
                ...
                for (size_t nc = 0; nc < (size_t) params->n; nc += n_chunk_n_cols) {
                    ...
                    {
                        if (params->weight_pre_tiled) {
                            const __fp16 * weight_chunk = hmx_pre_tiled_weight_chunk(weight_group, nc, params->k);
                            const size_t   weight_bytes = hmx_pre_tiled_weight_chunk_bytes(n_cols, params->k);
                            dma_queue_push(ctx->dma[0], dma_make_ptr(vtcm_weight, weight_chunk),
...
int hmx_mat_mul_permuted_w16a32(struct htp_context *ctx, float *restrict dst, const float *restrict activation,
                                ...
                                int act_stride, int weight_stride, int weight_pre_tiled) {
    if (!dst || !activation || !permuted_weight || !m || !n || !k) { return -1; }
    if (act_stride < k || (!weight_pre_tiled && weight_stride < k)) { return -1; }
    ...
    size_t m_chunk_n_rows = 0, n_chunk_n_cols = 0, vtcm_used = 0;
    // Pre-tiled weights skip per-matmul row-major scratch and interleave.
    const size_t weight_per_n = weight_pre_tiled ? vec_dot_size : 3 * vec_dot_size;
    if (hmx_compute_chunks(vtcm_budget,
                           /*overhead=*/256,
                           /*per_n=*/weight_per_n,                      // W or W + S0 + S1
    ...
    const size_t output_area_size     = hex_align_up(m_chunk_n_rows * n_chunk_n_cols * sizeof(__fp16), HMX_FP16_TILE_SIZE);
    const size_t scratch_area_size    = weight_pre_tiled ? 0 : hex_align_up(n_chunk_n_cols * vec_dot_size, HMX_FP16_TILE_SIZE);
    const size_t f32_scratch_size     = use_dma_activation
    ...
    __fp16  *vtcm_activation = (__fp16 *) vtcm_seq_alloc(&vtcm_ptr, activation_area_size);
    __fp16  *vtcm_output     = (__fp16 *) vtcm_seq_alloc(&vtcm_ptr, output_area_size);
    void    *vtcm_scratch0   = weight_pre_tiled ? NULL : vtcm_seq_alloc(&vtcm_ptr, scratch_area_size);
    void    *vtcm_scratch1   = weight_pre_tiled ? NULL : vtcm_seq_alloc(&vtcm_ptr, scratch_area_size);
    __fp16  *vtcm_scales     = (__fp16 *) vtcm_seq_alloc(&vtcm_ptr, 256);
    float   *vtcm_f32_act    = use_dma_activation ? (float *) vtcm_seq_alloc(&vtcm_ptr, f32_scratch_size) : NULL;
    if ((size_t)(vtcm_ptr - (uint8_t *)ctx->vtcm_base) > vtcm_budget) {
    ...
    FARF(MEDIUM, "%s: m=%d k=%d n=%d mc=%zu nc=%zu pretiled=%d vtcm=%zu/%zu",
         __func__, m, k, n, m_chunk_n_rows, n_chunk_n_cols, weight_pre_tiled,
         (size_t)(vtcm_ptr - (uint8_t *)ctx->vtcm_base), vtcm_budget);
    ...
    for (size_t mr = 0; mr < m; mr += m_chunk_n_rows) {
        ...
        for (size_t nc = 0; nc < n; nc += n_chunk_n_cols) {
            ...
            {
                if (weight_pre_tiled) {
                    const __fp16 * weight_chunk = hmx_pre_tiled_weight_chunk(permuted_weight, nc, k);
                    const size_t   weight_bytes = hmx_pre_tiled_weight_chunk_bytes(n_cols, k);
                    dma_queue_push(ctx->dma[0], dma_make_ptr(vtcm_weight, weight_chunk),
                                   weight_bytes, weight_bytes, weight_bytes, 1);
                    dma_queue_pop(ctx->dma[0]);
                } else {
