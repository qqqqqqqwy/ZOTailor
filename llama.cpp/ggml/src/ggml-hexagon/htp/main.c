...
static int execute_op(struct htp_ops_context * octx) {
    switch (octx->op) {
        ...
        case HTP_OP_LORA_MUL_MAT:
            return op_lora_mul_mat(octx);
...
static int proc_op_req(struct htp_ops_context * octx, struct htp_tensor *tens, uint32_t idx, struct htp_op_desc * op) {
    ...
    for (uint32_t i=0; i<HTP_OP_MAX_INPUTS; i++) {
        ...
        if (src->flags & HTP_TENSOR_HOST_DIRTY) {
            // Host-updated persistent tensors, such as LoRA B, must invalidate
            // DSP cache lines.  Clean+invalidate can write stale DSP data back
            // over the newer CPU-side update.
...
static void htp_packet_callback(dspqueue_t queue, int error, void * context) {
    ...
    while (!ctx->vtcm_needs_release) {
        ...
        if (r_size < 4u * sizeof(uint32_t) || n_dbufs != 1) {
        ...
        if (r_size != sizeof(req) ||
            req.abi_magic   != HTP_OPS_ABI_MAGIC ||
            req.abi_version != HTP_OPS_ABI_VERSION ||
            req.op_desc_size != sizeof(struct htp_op_desc) ||
            req.tensor_size  != sizeof(struct htp_tensor)) {
            FARF(ERROR, "invalid opbatch ABI: req-size %u/%zu magic 0x%x version %u op-desc %u/%zu tensor %u/%zu",
                    r_size, sizeof(req), req.abi_magic, req.abi_version,
                    req.op_desc_size, sizeof(struct htp_op_desc),
                    req.tensor_size, sizeof(struct htp_tensor));

            struct htp_opbatch_rsp rsp;
            memset(&rsp, 0, sizeof(rsp));
            rsp.status       = HTP_STATUS_ABI_MISMATCH;
            rsp.n_bufs       = n_bufs;
            rsp.n_tensors    = n_tens;
            rsp.n_ops        = n_ops;
            rsp.abi_magic    = HTP_OPS_ABI_MAGIC;
            rsp.abi_version  = HTP_OPS_ABI_VERSION;
            rsp.op_desc_size = sizeof(struct htp_op_desc);
            rsp.tensor_size  = sizeof(struct htp_tensor);
        ...
        if (dbuf.size < b_size + t_size + o_size) {
            ...
            struct htp_opbatch_rsp rsp;
            memset(&rsp, 0, sizeof(rsp));
            rsp.status       = HTP_STATUS_INVAL_PARAMS;
            rsp.n_bufs       = n_bufs;
            rsp.n_tensors    = n_tens;
            rsp.n_ops        = n_ops;
            rsp.abi_magic    = HTP_OPS_ABI_MAGIC;
        ...
        for (uint32_t i=0; i < n_ops; i++) {
            ...
            if (!htp_status_is_ok(op_status)) {
                FARF(ERROR, "op #%u failed: opcode %u status %d", i, ops[i].opcode, op_status);
