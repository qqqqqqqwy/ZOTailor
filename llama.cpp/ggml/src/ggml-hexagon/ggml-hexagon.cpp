...
enum ggml_hexagon_weight_layout {
    GGML_HEXAGON_WEIGHT_LAYOUT_AUTO,
    GGML_HEXAGON_WEIGHT_LAYOUT_LEGACY,
    GGML_HEXAGON_WEIGHT_LAYOUT_X4X2,
};

static ggml_hexagon_weight_layout opt_weight_layout = GGML_HEXAGON_WEIGHT_LAYOUT_AUTO;
static int opt_fused_lora  = 1; // auto/on by default, 0 disables GGML_OP_LORA_MUL_MAT
static int opt_lora_debug  = 0;
static int opt_lora_debug_deep = 0;
static int opt_lora_profile = 0;
static int opt_lora_fast   = 1;
static int opt_split_trace = 0;
static int opt_flash_attn_prompt = 0; // opt-in until prompt FA is validated per SoC/model
static int opt_rpcmem_uncached = 0;
...
struct ggml_hexagon_shared_buffer {
    ...
    bool                   mapped;
    bool                   pinned;
    std::unordered_map<uint64_t, uint32_t> tensor_flags;
    ...
    void alloc(size_t size, bool pinned = false) {
        ...
        const uint32_t rpcmem_flags = opt_rpcmem_uncached ? RPCMEM_FLAG_UNCACHED : RPCMEM_DEFAULT_FLAGS;
        this->base = (uint8_t *) rpcmem_alloc2(RPCMEM_HEAP_ID_SYSTEM, rpcmem_flags, size);
        if (!this->base) {
        ...
        HEX_VERBOSE("ggml-hex: %s allocated buffer: base %p size %zu fd %d pinned %d rpcmem-flags 0x%x\n", sess->c_name(),
                    (void *) this->base, this->size, this->fd, (int) pinned, rpcmem_flags);
...
static bool ggml_hexagon_str_contains(const char * s, const char * needle) {
    return s && strstr(s, needle) != NULL;
}

static bool ggml_hexagon_is_lora_tensor(const ggml_tensor * tensor) {
    return ggml_hexagon_str_contains(tensor->name, ".lora_a") ||
           ggml_hexagon_str_contains(tensor->name, ".lora_b") ||
           ggml_hexagon_str_contains(tensor->name, ".lora_b_plus") ||
           ggml_hexagon_str_contains(tensor->name, ".lora_b_minus");
}

static bool ggml_hexagon_is_lora_b_tensor(const ggml_tensor * tensor) {
    return ggml_hexagon_str_contains(tensor->name, ".lora_b");
}

static bool ggml_hexagon_is_lora_dynamic_tensor(const ggml_tensor * tensor) {
    return ggml_hexagon_str_contains(tensor->name, ".lora_b") ||
           ggml_hexagon_str_contains(tensor->name, ".lora_b_plus") ||
           ggml_hexagon_str_contains(tensor->name, ".lora_b_minus");
}

static bool ggml_hexagon_is_llama_matmul_weight(const ggml_tensor * tensor) {
    const char * name = tensor->name;
    return ggml_hexagon_str_contains(name, "attn_q.weight")      ||
           ggml_hexagon_str_contains(name, "attn_k.weight")      ||
           ggml_hexagon_str_contains(name, "attn_v.weight")      ||
           ggml_hexagon_str_contains(name, "attn_output.weight") ||
           ggml_hexagon_str_contains(name, "ffn_up.weight")      ||
           ggml_hexagon_str_contains(name, "ffn_down.weight")    ||
           ggml_hexagon_str_contains(name, "ffn_gate.weight");
}

static bool ggml_hexagon_can_pre_tile_f16_weight(const ggml_tensor * tensor) {
    if (opt_weight_layout == GGML_HEXAGON_WEIGHT_LAYOUT_X4X2 || !opt_use_hmx) {
        return false;
    }

    if (ggml_hexagon_is_lora_tensor(tensor)) {
        return false;
    }
    ...
    return tensor->type == GGML_TYPE_F16 &&
           ...
           ggml_hexagon_is_llama_matmul_weight(tensor);
}

static uint64_t ggml_hexagon_tensor_offset(const ggml_hexagon_shared_buffer * sbuf, const ggml_tensor * tensor) {
    return (uint8_t *) tensor->data - sbuf->base;
}

static const char * ggml_hexagon_host_write_sync(ggml_hexagon_shared_buffer * sbuf, void * ptr, size_t size, bool start) {
    if (size == 0) {
        return "none";
    }
    ...
    if (!dma_sync_warned.exchange(true)) {
        GGML_LOG_WARN("ggml-hex: DMA_BUF_IOCTL_SYNC(%s|WRITE) failed for fd %d (%s); falling back to msync on write end\n",
                start ? "START" : "END", sbuf->fd, strerror(errno));
    }
#else
    GGML_UNUSED(sbuf);
#endif
    ...
    const uintptr_t begin = (uintptr_t) ptr & ~page_mask;
    const uintptr_t end = ((uintptr_t) ptr + size + page_mask) & ~page_mask;
    if (msync((void *) begin, end - begin, MS_SYNC) != 0) {
        static std::atomic<bool> msync_warned{false};
        if (!msync_warned.exchange(true)) {
            GGML_LOG_WARN("ggml-hex: msync fallback failed for host write (%s)\n", strerror(errno));
        }
        return "failed";
    }
    return "msync";
...
// Convert a row-major F16 [n, k] weight matrix into the exact tile-major layout
// consumed by core_dot_chunk_fp16(). This mirrors interleave_fp16_weight_chunk_to_tiles()
// in the HTP binary, but pays the cost once during tensor upload/offload.
static void repack_f16_hmx_tiles(ggml_tensor * tensor, const void * data, size_t size) {
    GGML_ASSERT(tensor->type == GGML_TYPE_F16);
    GGML_ASSERT(size == ggml_nbytes(tensor));

    const int64_t k = tensor->ne[0];
    const int64_t n = tensor->ne[1];
    GGML_ASSERT(k % 32 == 0 && n % 32 == 0);

    const ggml_fp16_t * src = (const ggml_fp16_t *) data;
    ggml_fp16_t *       dst = (ggml_fp16_t *) tensor->data;

    const int64_t n_k_tiles = k / 32;
    for (int64_t ct = 0; ct < n / 32; ++ct) {
        for (int64_t kt = 0; kt < n_k_tiles; ++kt) {
            ggml_fp16_t * tile = dst + (ct * n_k_tiles + kt) * 1024;
            for (int64_t r = 0; r < 32; ++r) {
                const ggml_fp16_t * row = src + (ct * 32 + r) * k + kt * 32;
                for (int64_t p = 0; p < 16; ++p) {
                    tile[p * 64 + r * 2 + 0] = row[p * 2 + 0];
                    tile[p * 64 + r * 2 + 1] = row[p * 2 + 1];
                }
            }
        }
    }
...
static void unpack_f16_hmx_tiles(void * data, const ggml_tensor * tensor, size_t size) {
    GGML_ASSERT(tensor->type == GGML_TYPE_F16);
    GGML_ASSERT(size == ggml_nbytes(tensor));
    ...
    for (int64_t ct = 0; ct < n / 32; ++ct) {
        for (int64_t kt = 0; kt < n_k_tiles; ++kt) {
            const ggml_fp16_t * tile = src + (ct * n_k_tiles + kt) * 1024;
            for (int64_t r = 0; r < 32; ++r) {
                ggml_fp16_t * row = dst + (ct * 32 + r) * k + kt * 32;
                for (int64_t p = 0; p < 16; ++p) {
                    row[p * 2 + 0] = tile[p * 64 + r * 2 + 0];
                    row[p * 2 + 1] = tile[p * 64 + r * 2 + 1];
                }
            }
        }
    }
...
static inline void hexagon_set_tensor_raw_with_lora_sync(ggml_hexagon_shared_buffer * sbuf,
                                                         ggml_tensor *                tensor,
                                                         uint64_t                     t_offset,
                                                         const void *                 data,
                                                         size_t                       offset,
    ...
    const bool lora_dynamic_upload = ggml_hexagon_is_lora_dynamic_tensor(tensor);
    const char * sync_start = "none";
    const char * sync_end   = "none";
    if (lora_dynamic_upload) {
        sync_start = ggml_hexagon_host_write_sync(sbuf, dst, size, true);
    }
    memcpy(dst, data, size);
    if (lora_dynamic_upload) {
        sync_end = ggml_hexagon_host_write_sync(sbuf, dst, size, false);
    }
    if (offset == 0 && lora_dynamic_upload) {
        sbuf->tensor_flags[t_offset] |= HTP_TENSOR_HOST_DIRTY;
    }
    if (lora_dynamic_upload && opt_lora_debug) {
        static std::atomic<int> lora_upload_logs{0};
        const int n_logs = lora_upload_logs.fetch_add(1);
        if (n_logs < 512) {
            GGML_LOG_INFO("ggml-hex: %s lora-dynamic upload %s offset %zu size %zu sync-start %s sync-end %s flags 0x%x\n",
                    sess->c_name(), tensor->name, offset, size, sync_start, sync_end,
                    (unsigned) sbuf->tensor_flags[t_offset]);
        }
    }
}
...
static void ggml_backend_hexagon_buffer_set_tensor(ggml_backend_buffer_t buffer,
    ...
    const uint64_t t_offset = ggml_hexagon_tensor_offset(sbuf, tensor);
    if (offset == 0) {
        sbuf->tensor_flags.erase(t_offset);
    }

    switch (tensor->type) {
        case GGML_TYPE_F16:
            if (ggml_hexagon_can_pre_tile_f16_weight(tensor)) {
                GGML_ASSERT(offset == 0);
                GGML_ASSERT(size == ggml_nbytes(tensor));
                repack_f16_hmx_tiles(tensor, data, size);
                sbuf->tensor_flags[t_offset] = HTP_TENSOR_WEIGHT_PRETILED;
                HEX_VERBOSE("ggml-hex: %s set-tensor %s : legacy-pre-tiled-f16\n", sess->c_name(), tensor->name);
            } else {
                hexagon_set_tensor_raw_with_lora_sync(sbuf, tensor, t_offset, data, offset, size);
            }
            break;
...
static void ggml_backend_hexagon_buffer_get_tensor(ggml_backend_buffer_t buffer,
    ...
    const uint64_t t_offset = ggml_hexagon_tensor_offset(sbuf, tensor);
    const auto flags_it = sbuf->tensor_flags.find(t_offset);
    if (flags_it != sbuf->tensor_flags.end() && (flags_it->second & HTP_TENSOR_WEIGHT_PRETILED)) {
        GGML_ASSERT(tensor->type == GGML_TYPE_F16);
        GGML_ASSERT(offset == 0);
        GGML_ASSERT(size == ggml_nbytes(tensor));
        unpack_f16_hmx_tiles(data, tensor, size);
        return;
    }
...
struct ggml_hexagon_opbatch {
    ...
    int add_tensor(const ggml_tensor * t) {
        ...
        auto flags_it = sbuf->tensor_flags.find(t_offset);
        if (flags_it != sbuf->tensor_flags.end()) {
            h.flags |= flags_it->second;
        }

        if (opt_lora_debug && ggml_hexagon_is_lora_dynamic_tensor(t)) {
            static std::atomic<int> lora_tensor_logs{0};
            const int n_logs = lora_tensor_logs.fetch_add(1);
            if (n_logs < (opt_lora_debug_deep ? 512 : 32)) {
                double sum_abs_host = 0.0;
                if (opt_lora_debug_deep && t->type == GGML_TYPE_F16) {
                    const ggml_fp16_t * src = (const ggml_fp16_t *) t->data;
                    const size_t n = (size_t) ggml_nelements(t);
                    for (size_t i = 0; i < n; ++i) {
                        sum_abs_host += std::fabs((double) ggml_fp16_to_fp32(src[i]));
                    }
                }
                GGML_LOG_INFO("ggml-hex: %s lora-dynamic tensor-desc #%u %s bi %d offset %zu size %zu flags 0x%x debug-deep %d sum_abs_host %.8e\n",
                        name, ti, t->name, h.bi, (size_t) t_offset, t_size, h.flags, opt_lora_debug_deep, sum_abs_host);
            }
        }
    ...
    void add_op(htp_op_code opcode, const struct ggml_tensor * t) {
        ...
        memcpy(o.params, t->op_params, sizeof(o.params));
        o.opcode = opcode;
        o.flags  = 0;
        ...
        if (opcode == HTP_OP_LORA_MUL_MAT) {
            struct lora_op_params {
                float   scale;
                int32_t positive_rows;
                int32_t flags;
            } lora_params = {};
            memcpy(&lora_params, t->op_params, sizeof(lora_params));

            const bool lora_antithetic = t->src[4] != nullptr;
            o.lora_scale = lora_params.scale;
            if (lora_antithetic) {
                o.flags |= HTP_OPFLAGS_LORA_ANTITHETIC;
                o.lora_positive_rows = lora_params.positive_rows > 0 ? (uint32_t) lora_params.positive_rows : 0;
            }
            if (lora_params.flags & (1 << 1)) {
                o.flags |= HTP_OPFLAGS_LORA_B_RM_F32;
            }

            if (opt_lora_fast) {
                o.flags |= HTP_OPFLAGS_LORA_FAST;
            }
            if (opt_lora_debug_deep) {
                o.flags |= HTP_OPFLAGS_LORA_DEBUG_DEEP;
            }
            if (opt_lora_profile) {
                o.flags |= HTP_OPFLAGS_LORA_PROFILE;
            }

            if (opt_lora_debug) {
                o.flags |= HTP_OPFLAGS_LORA_DEBUG;
                static std::atomic<int> lora_enqueue_logs{0};
                const int n_logs = lora_enqueue_logs.fetch_add(1);
                if (n_logs < (opt_lora_debug_deep ? 256 : 32)) {
                    const ggml_tensor * b = t->src[3];
                    double sum_abs_b = 0.0;
                    if (opt_lora_debug_deep && b && b->type == GGML_TYPE_F16) {
                        const ggml_fp16_t * src = (const ggml_fp16_t *) b->data;
                        const size_t n = (size_t) ggml_nelements(b);
                        for (size_t i = 0; i < n; ++i) {
                            sum_abs_b += std::fabs((double) ggml_fp16_to_fp32(src[i]));
                        }
                    }
                    GGML_LOG_INFO("ggml-hex: %s enqueue LORA_MUL_MAT scale %.8g positive_rows %u antithetic %d flags 0x%x fast %d debug-deep %d b_sum_abs %.8e : %s\n",
                            name, (double) o.lora_scale, o.lora_positive_rows,
                            lora_antithetic ? 1 : 0, o.flags, opt_lora_fast, opt_lora_debug_deep, sum_abs_b, t->name);
                }
            }
        }
...
void ggml_hexagon_session::flush_pending(bool all) {
    while (this->op_pending) {
        ...
        if (rsp_size != sizeof(rsp) || n_dbufs != 1) {
            if (n_dbufs == 1 && dbuf.ptr) {
                op_shm->release((uint8_t*) dbuf.ptr);
            }
            GGML_ABORT("ggml-hex: %s dspcall : bad response / ABI mismatch : size %u/%zu dspbufs %u\n",
                    this->c_name(), rsp_size, sizeof(rsp), n_dbufs);
        }

        if (rsp.abi_magic != HTP_OPS_ABI_MAGIC ||
            rsp.abi_version != HTP_OPS_ABI_VERSION ||
            rsp.op_desc_size != sizeof(htp_op_desc) ||
            rsp.tensor_size != sizeof(htp_tensor)) {
            op_shm->release((uint8_t*) dbuf.ptr);
            GGML_ABORT("ggml-hex: %s dspcall : op ABI mismatch : status %s magic 0x%x version %u op-desc %u/%zu tensor %u/%zu. Clean rebuild and reinstall libggml-hexagon.so/libggml-htp-vXX.so together.\n",
                    this->c_name(), status_to_str(rsp.status), rsp.abi_magic, rsp.abi_version,
                    rsp.op_desc_size, sizeof(htp_op_desc), rsp.tensor_size, sizeof(htp_tensor));
        }

        if ((opt_lora_debug || opt_lora_profile) && rsp.n_ops > 0) {
            ...
            if (dbuf.ptr && dbuf.size >= b_size + t_size + o_size) {
                htp_op_desc * ops = (htp_op_desc *) ((uint8_t *) dbuf.ptr + b_size + t_size);
                static std::atomic<int> lora_debug_logs{0};
                static std::atomic<int> lora_debug_batch_logs{0};
                static std::atomic<int> lora_profile_logs{0};
                static std::atomic<int> lora_profile_batch_logs{0};
                uint32_t n_lora_stats = 0;
                uint32_t n_lora_profile = 0;
                double sum_abs_tmp_total     = 0.0;
                double sum_abs_b_total       = 0.0;
                double sum_abs_b_minus_total = 0.0;
                double sum_abs_delta_total   = 0.0;
                uint64_t prof_op_total_us    = 0;
                uint64_t prof_main_total_us  = 0;
                uint64_t prof_tmp_total_us   = 0;
                uint64_t prof_bprep_total_us = 0;
                uint64_t prof_acc_total_us   = 0;
                for (uint32_t i = 0; i < rsp.n_ops; ++i) {
                    if (opt_lora_debug &&
                        ops[i].opcode == HTP_OP_LORA_MUL_MAT &&
                        (ops[i].debug_flags & HTP_OP_DEBUG_LORA_STATS)) {
                        n_lora_stats++;
                        sum_abs_tmp_total     += ops[i].debug_sum_abs_tmp;
                        sum_abs_b_total       += ops[i].debug_sum_abs_b;
                        sum_abs_b_minus_total += ops[i].debug_sum_abs_b_minus;
                        sum_abs_delta_total   += ops[i].debug_sum_abs_delta;

                        const int n_logs = lora_debug_logs.fetch_add(1);
                        if (n_logs < 64) {
                            GGML_LOG_INFO("ggml-hex: %s lora-debug op %u status %s stage %u fast %u deep %u scale %.8g k %u r %u m %u n %u sum_abs_tmp %.8e sum_abs_b %.8e sum_abs_b_minus %.8e sum_abs_delta %.8e\n",
                                    this->c_name(), i,
                                    status_to_str(ops[i].debug_status), ops[i].debug_stage,
                                    (ops[i].flags & HTP_OPFLAGS_LORA_FAST) ? 1u : 0u,
                                    (ops[i].flags & HTP_OPFLAGS_LORA_DEBUG_DEEP) ? 1u : 0u,
                                    (double) ops[i].debug_scale,
                                    ops[i].debug_k, ops[i].debug_r, ops[i].debug_m, ops[i].debug_n,
                                    (double) ops[i].debug_sum_abs_tmp,
                                    (double) ops[i].debug_sum_abs_b,
                                    (double) ops[i].debug_sum_abs_b_minus,
                                    (double) ops[i].debug_sum_abs_delta);
                        }
                    }

                    if (opt_lora_profile &&
                        ops[i].opcode == HTP_OP_LORA_MUL_MAT &&
                        (ops[i].flags & HTP_OPFLAGS_LORA_PROFILE)) {
                        n_lora_profile++;
                        prof_op_total_us    += ops[i].prof_usecs;
                        prof_main_total_us  += ops[i].lora_prof_main_us;
                        prof_tmp_total_us   += ops[i].lora_prof_tmp_us;
                        prof_bprep_total_us += ops[i].lora_prof_bprep_us;
                        prof_acc_total_us   += ops[i].lora_prof_acc_us;

                        const int n_logs = lora_profile_logs.fetch_add(1);
                        if (n_logs < 64) {
                            GGML_LOG_INFO("ggml-hex: %s lora-profile op %u total %u us main %u us tmp %u us bprep %u us acc %u us antithetic %u m %u pos_rows %u\n",
                                    this->c_name(), i, ops[i].prof_usecs,
                                    ops[i].lora_prof_main_us,
                                    ops[i].lora_prof_tmp_us,
                                    ops[i].lora_prof_bprep_us,
                                    ops[i].lora_prof_acc_us,
                                    (ops[i].flags & HTP_OPFLAGS_LORA_ANTITHETIC) ? 1u : 0u,
                                    ops[i].debug_m, ops[i].lora_positive_rows);
                        }
                    }
                }
                if (opt_lora_debug && n_lora_stats > 0) {
                    const int n_batch_logs = lora_debug_batch_logs.fetch_add(1);
                    if (n_batch_logs < 2048) {
                        GGML_LOG_INFO("ggml-hex: %s lora-debug batch n_ops %u n_lora %u deep %d sum_abs_tmp_total %.8e sum_abs_b_total %.8e sum_abs_b_minus_total %.8e sum_abs_delta_total %.8e\n",
                                this->c_name(), rsp.n_ops, n_lora_stats,
                                opt_lora_debug_deep, sum_abs_tmp_total, sum_abs_b_total, sum_abs_b_minus_total, sum_abs_delta_total);
                    }
                } else if (opt_lora_debug) {
                    static std::atomic<int> lora_debug_empty_logs{0};
                    const int n_empty_logs = lora_debug_empty_logs.fetch_add(1);
                    if (n_empty_logs < 16) {
                        GGML_LOG_WARN("ggml-hex: %s lora-debug found no LORA_MUL_MAT stats in completed batch with %u ops\n",
                                this->c_name(), rsp.n_ops);
                    }
                }
                if (opt_lora_profile && n_lora_profile > 0) {
                    const int n_batch_logs = lora_profile_batch_logs.fetch_add(1);
                    if (n_batch_logs < 2048) {
                        GGML_LOG_INFO("ggml-hex: %s lora-profile batch n_ops %u n_lora %u total %llu us main %llu us tmp %llu us bprep %llu us acc %llu us\n",
                                this->c_name(), rsp.n_ops, n_lora_profile,
                                (unsigned long long) prof_op_total_us,
                                (unsigned long long) prof_main_total_us,
                                (unsigned long long) prof_tmp_total_us,
                                (unsigned long long) prof_bprep_total_us,
                                (unsigned long long) prof_acc_total_us);
                    }
                }
            } else {
                static std::atomic<bool> lora_debug_bad_rsp_warned{false};
                if (!lora_debug_bad_rsp_warned.exchange(true)) {
                    GGML_LOG_WARN("ggml-hex: %s lora debug/profile could not parse opbatch response: ptr %p size %u counts %u/%u/%u\n",
                            this->c_name(), dbuf.ptr, dbuf.size, rsp.n_bufs, rsp.n_tensors, rsp.n_ops);
                }
            }
        }
        ...
        if (!htp_status_is_ok(rsp.status)) {
            GGML_ABORT("ggml-hex: %s dspcall : dsp-rsp: %s\n", this->c_name(), status_to_str(rsp.status));
        }
...
void ggml_hexagon_session::flush_batch() {
    ...
    req.flags        = 0;
    req.abi_magic    = HTP_OPS_ABI_MAGIC;
    req.abi_version  = HTP_OPS_ABI_VERSION;
    req.op_desc_size = sizeof(htp_op_desc);
    req.tensor_size  = sizeof(htp_tensor);
...
static inline bool ggml_hexagon_is_plain_2d_batch(const struct ggml_tensor * t) {
    return t->ne[2] == 1 &&
           t->ne[3] == 1 &&
           ggml_nrows(t) == t->ne[1] &&
...
static inline bool ggml_hexagon_is_prompt_batch_supported(const struct ggml_tensor * src1) {
    return ggml_hexagon_is_plain_2d_batch(src1) && ggml_nrows(src1) <= 4096;
}
...
static bool ggml_hexagon_supported_flash_attn_ext(const struct ggml_hexagon_session * sess, const struct ggml_tensor * op) {
    ...
    if ((dst->ne[2] != 1 || dst->ne[3] != 1) && !opt_flash_attn_prompt) {
        return false;
    }

    if (src1->ne[2] == 0 || src2->ne[2] == 0 ||
        src0->ne[2] % src1->ne[2] != 0 ||
        src0->ne[2] % src2->ne[2] != 0 ||
        src0->ne[3] % src1->ne[3] != 0 ||
        src0->ne[3] % src2->ne[3] != 0) {
        return false;
    }
...
static bool ggml_hexagon_supported_mul_mat(const struct ggml_hexagon_session * sess, const struct ggml_tensor * dst) {
    ...
    if (!ggml_hexagon_is_plain_2d_batch(dst) || !ggml_hexagon_is_prompt_batch_supported(src1)) {
        return false;
    }
...
static bool ggml_hexagon_supported_lora_mul_mat(const struct ggml_hexagon_session * sess, const struct ggml_tensor * dst) {
    GGML_UNUSED(sess);
    ...
    const struct ggml_tensor * lora_a = dst->src[2];
    const struct ggml_tensor * lora_b = dst->src[3];
    const struct ggml_tensor * lora_b_minus = dst->src[4];
    const bool antithetic = lora_b_minus != nullptr;
    struct lora_op_params {
        float   scale;
        int32_t positive_rows;
        int32_t flags;
    } lora_params = {};
    memcpy(&lora_params, dst->op_params, sizeof(lora_params));
    const bool lora_b_rm_f32 = antithetic && (lora_params.flags & (1 << 1));

    if (!w || !x || !lora_a || !lora_b) {
        return false;
    }

    if (dst->type != GGML_TYPE_F32 ||
        (w->type != GGML_TYPE_F16 && w->type != GGML_TYPE_Q4_0) ||
        lora_a->type != GGML_TYPE_F16 ||
        (!lora_b_rm_f32 && lora_b->type != GGML_TYPE_F16) ||
        (lora_b_rm_f32 && lora_b->type != GGML_TYPE_F32) ||
        (antithetic && !lora_b_rm_f32 && lora_b_minus->type != GGML_TYPE_F16) ||
        (antithetic && lora_b_rm_f32 && lora_b_minus->type != GGML_TYPE_F32)) {
        return false;
    }
    ...
    if (w->type == GGML_TYPE_Q4_0) {
        ...
        if (w->buffer && !ggml_backend_buffer_is_hexagon_repack(w->buffer)) {
            return false;
        }
    }
    ...
    if (!ggml_hexagon_is_plain_2d_batch(dst) || !ggml_hexagon_is_prompt_batch_supported(x)) {
        return false;
    }

    if (!ggml_hexagon_is_plain_2d_batch(w) ||
        !ggml_hexagon_is_plain_2d_batch(lora_a) ||
        !ggml_hexagon_is_plain_2d_batch(lora_b) ||
        (antithetic && !ggml_hexagon_is_plain_2d_batch(lora_b_minus))) {
        return false;
    }

    const int64_t k = w->ne[0];
    const int64_t n = w->ne[1];
    const int64_t r = lora_a->ne[1];
    ...
    } else if (lora_b->ne[0] != r ||
               lora_b->ne[1] != n) {
        return false;
    }

    if (antithetic) {
        if (lora_b_minus->ne[0] != lora_b->ne[0] ||
            lora_b_minus->ne[1] != lora_b->ne[1] ||
            lora_b_minus->ne[2] != lora_b->ne[2] ||
            lora_b_minus->ne[3] != lora_b->ne[3] ||
            lora_params.positive_rows <= 0 ||
            (int64_t) lora_params.positive_rows * 2 != x->ne[1]) {
            return false;
        }
    }
...
static htp_op_code op_remap_to_htp(const ggml_tensor * t) {
    switch (t->op) {
        ...
        case GGML_OP_LORA_MUL_MAT:   return HTP_OP_LORA_MUL_MAT;
...
static bool ggml_backend_hexagon_device_supports_op(ggml_backend_dev_t dev, const struct ggml_tensor * op) {
    ...
    if (!ggml_hexagon_supported_buffers(sess, op)) {
        ...
        if (opt_split_trace) {
            op_desc desc(op);
            GGML_LOG_INFO("ggml-hex: %s split-trace reject %s : buffers : %s : %s : %s : %s : %s\n",
                    sess->name.c_str(), ggml_op_desc(op), desc.names, desc.dims, desc.types, desc.strides, desc.buffs);
        }
        return false;
    }
    ...
    switch (op->op) {
        ...
        case GGML_OP_LORA_MUL_MAT:
            supp = ggml_hexagon_supported_lora_mul_mat(sess, op);
            break;
    ...
    if (opt_split_trace && !supp) {
        op_desc desc(op);
        GGML_LOG_INFO("ggml-hex: %s split-trace reject %s : %s : %s : %s : %s : %s\n",
                sess->name.c_str(), ggml_op_desc(op), desc.names, desc.dims, desc.types, desc.strides, desc.buffs);
    }
    return supp;
}
...
static void ggml_hexagon_init(ggml_backend_reg * reg) {
    ...
    const char * str_weight_layout = getenv("GGML_HEXAGON_WEIGHT_LAYOUT");
    const char * str_fused_lora = getenv("GGML_HEXAGON_FUSED_LORA");
    const char * str_lora_debug = getenv("GGML_HEXAGON_LORA_DEBUG");
    const char * str_lora_debug_deep = getenv("GGML_HEXAGON_LORA_DEBUG_DEEP");
    const char * str_lora_profile = getenv("GGML_HEXAGON_LORA_PROFILE");
    const char * str_lora_fast = getenv("GGML_HEXAGON_LORA_FAST");
    const char * str_split_trace = getenv("GGML_HEXAGON_SPLIT_TRACE");
    const char * str_flash_attn_prompt = getenv("GGML_HEXAGON_FLASH_ATTN_PROMPT");
    const char * str_rpcmem_uncached = getenv("GGML_HEXAGON_RPCMEM_UNCACHED");
    ...
    opt_lora_debug   = str_lora_debug ? atoi(str_lora_debug) : opt_lora_debug;
    opt_lora_debug_deep = str_lora_debug_deep ? atoi(str_lora_debug_deep) : opt_lora_debug_deep;
    opt_lora_profile = str_lora_profile ? atoi(str_lora_profile) : opt_lora_profile;
    if (opt_lora_debug_deep) {
        opt_lora_debug = 1;
    }
    opt_lora_fast    = str_lora_fast ? atoi(str_lora_fast) : opt_lora_fast;
    opt_split_trace  = str_split_trace ? atoi(str_split_trace) : opt_split_trace;
    opt_flash_attn_prompt = str_flash_attn_prompt ? atoi(str_flash_attn_prompt) : opt_flash_attn_prompt;
    opt_rpcmem_uncached = str_rpcmem_uncached ? atoi(str_rpcmem_uncached) : opt_rpcmem_uncached;
    opt_ndev         = str_ndev    ? strtoul(str_ndev, NULL, 0) : opt_ndev;
    opt_hostbuf      = str_hostbuf ? atoi(str_hostbuf) : opt_hostbuf;

    if (opt_rpcmem_uncached) {
        GGML_LOG_WARN("ggml-hex: GGML_HEXAGON_RPCMEM_UNCACHED=1, allocating Hexagon shared buffers uncached\n");
    }

    if (str_fused_lora) {
        if (strcmp(str_fused_lora, "0") == 0 || strcmp(str_fused_lora, "off") == 0 ||
            strcmp(str_fused_lora, "false") == 0 || strcmp(str_fused_lora, "runtime") == 0) {
            opt_fused_lora = 0;
        } else if (strcmp(str_fused_lora, "auto") == 0 || strcmp(str_fused_lora, "1") == 0 ||
                   strcmp(str_fused_lora, "on") == 0 || strcmp(str_fused_lora, "true") == 0 ||
                   strcmp(str_fused_lora, "fused-htp") == 0) {
            opt_fused_lora = 1;
        } else {
            GGML_LOG_WARN("ggml-hex: unknown GGML_HEXAGON_FUSED_LORA=%s, using auto\n", str_fused_lora);
            opt_fused_lora = 1;
        }
    }

    if (str_weight_layout) {
        if (strcmp(str_weight_layout, "auto") == 0) {
            opt_weight_layout = GGML_HEXAGON_WEIGHT_LAYOUT_AUTO;
        } else if (strcmp(str_weight_layout, "legacy") == 0) {
            opt_weight_layout = GGML_HEXAGON_WEIGHT_LAYOUT_LEGACY;
        } else if (strcmp(str_weight_layout, "x4x2") == 0) {
            opt_weight_layout = GGML_HEXAGON_WEIGHT_LAYOUT_X4X2;
        } else {
            GGML_LOG_WARN("ggml-hex: unknown GGML_HEXAGON_WEIGHT_LAYOUT=%s, using auto\n", str_weight_layout);
            opt_weight_layout = GGML_HEXAGON_WEIGHT_LAYOUT_AUTO;
        }
    }
