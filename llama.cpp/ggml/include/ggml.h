...
extern "C" {
    ...
    // fused LoRA path: result = a*b + scale * lora_b*(lora_a*b)
    GGML_API struct ggml_tensor * ggml_lora_mul_mat(
            struct ggml_context * ctx,
            struct ggml_tensor  * a,
            struct ggml_tensor  * b,
    ...
    // antithetic fused LoRA path:
    // result = a*b + scale * select(lora_b_plus, lora_b_minus) * (lora_a*b)
    // rows [0, positive_rows) use lora_b_plus, remaining rows use lora_b_minus.
    GGML_API struct ggml_tensor * ggml_lora_mul_mat_antithetic(
            struct ggml_context * ctx,
            struct ggml_tensor  * a,
            struct ggml_tensor  * b,
            struct ggml_tensor  * lora_a,
    ...
    // antithetic fused LoRA path with rank-major F32 B tensors:
    // lora_b_*_rm shape is [output_dim, rank], stored as b_rm[rank_id * output_dim + output_id].
    GGML_API struct ggml_tensor * ggml_lora_mul_mat_antithetic_rm(
            struct ggml_context * ctx,
            struct ggml_tensor  * a,
            struct ggml_tensor  * b,
            struct ggml_tensor  * lora_a,
