...
- For a complete TinyLlama workflow covering `HF -> F16 GGUF -> Q4_0/Q8_0`, plus separate CPU-only and Hexagon prefill benchmark instructions on Android, see [tinyllama-android.md](tinyllama-android.md).
...
Prefill benchmark for quantized models via `llama-batched-bench`

```
~/src/llama.cpp$ M=Llama-3.2-1B-Instruct-Q4_0.gguf ./scripts/snapdragon/adb/run-tool.sh llama-batched-bench \
    -m /data/local/tmp/gguf/$M -ngl 99 --device none -c 4096 -b 2048 -ub 512 -npp 128,256,512 -ntg 0 -npl 1,2,4,8

~/src/llama.cpp$ M=Llama-3.2-1B-Instruct-Q4_0.gguf D=HTP0 ./scripts/snapdragon/adb/run-tool.sh llama-batched-bench \
    -m /data/local/tmp/gguf/$M -ngl 99 --device HTP0 -c 4096 -b 2048 -ub 512 -npp 128,256,512 -ntg 0 -npl 1,2,4,8
```

These runs focus on prompt processing (`-ntg 0`) and are the recommended first check for Hexagon prefill offload.
For `Q4_0` and `Q8_0` models the backend allocates an `HTP*-REPACK` buffer and repacks weights at load time.
For prompt-heavy tuning, `GGML_HEXAGON_OPBATCH` and `GGML_HEXAGON_OPQUEUE` are useful knobs to test larger Hexagon op batches.
...
- `GGML_HEXAGON_FUSED_LORA=auto|0|1`
  Controls the experimental F16 LoRA fused prefill path. `auto` and `1` allow eligible `LORA_MUL_MAT` ops to run on HTP; `0` keeps the standard runtime LoRA graph.

- `GGML_HEXAGON_SPLIT_TRACE=1`
  Logs unsupported ops reported by the Hexagon backend. Use this with `GGML_HEXAGON_VERBOSE=1` when investigating why the scheduler splits a graph.
  With `scripts/snapdragon/adb/run-tool.sh`, pass this as `SPLIT_TRACE=1 V=1`.

- `GGML_HEXAGON_FLASH_ATTN_PROMPT=0|1`
  Enables the experimental prompt FlashAttention support path. The default is `0`; set it to `1` when testing graph split reduction on prompt-heavy runs.
  With `scripts/snapdragon/adb/run-tool.sh`, pass this as `PROMPT_FA=1`.

- `GGML_HEXAGON_LORA_DEBUG=1`
  Prints lightweight fused LoRA DSP-side stats for the first few fused ops, including `scale`, `sum_abs_tmp`, `sum_abs_b`, and `sum_abs_delta`.
  With `scripts/snapdragon/adb/run-tool.sh`, pass this as `LORA_DEBUG=1 V=1`.

- `GGML_HEXAGON_RPCMEM_UNCACHED=1`
  Allocates Hexagon rpcmem buffers as uncached memory. This is a diagnostic fallback for host-updated tensors such as LoRA B when dma-buf cache sync is unavailable or suspected to be ineffective.
  With `scripts/snapdragon/adb/run-tool.sh`, pass this as `RPCMEM_UNCACHED=1`.
