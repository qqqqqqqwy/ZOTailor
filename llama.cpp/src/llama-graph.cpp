...
static bool llama_hexagon_fused_lora_enabled() {
    const char * debug_branch = std::getenv("GGML_LORA_DEBUG_BRANCH");
    if (debug_branch != nullptr &&
            strcmp(debug_branch, "0") != 0 &&
            strcmp(debug_branch, "off") != 0 &&
            strcmp(debug_branch, "false") != 0) {
        return false;
    }
    const char * env = std::getenv("GGML_HEXAGON_FUSED_LORA");
    if (env == nullptr) {
        return true; // default auto
    }

    if (strcmp(env, "0") == 0 || strcmp(env, "off") == 0 || strcmp(env, "false") == 0 || strcmp(env, "runtime") == 0) {
        return false;
    }
...
static bool llama_lora_debug_branch_enabled() {
    const char * env = std::getenv("GGML_LORA_DEBUG_BRANCH");
    return env != nullptr &&
           strcmp(env, "0") != 0 &&
           strcmp(env, "off") != 0 &&
           strcmp(env, "false") != 0;
...
static bool llama_hexagon_antithetic_lora_enabled() {
    const char * env = std::getenv("GGML_HEXAGON_ANTITHETIC_LORA");
    if (env == nullptr) {
        return false;
    }

    return strcmp(env, "0") != 0 && strcmp(env, "off") != 0 && strcmp(env, "false") != 0;
}

static bool llama_hexagon_fused_lora_weight_name_supported(const ggml_tensor * w) {
...
static bool llama_hexagon_name_contains(const char * name, const char * needle) {
    return name != nullptr && std::strstr(name, needle) != nullptr;
}

static bool llama_hexagon_tensor_is_resident(const ggml_tensor * t) {
    if (t == nullptr || t->buffer == nullptr) {
        return false;
    }

    ggml_backend_buffer_type_t buft = ggml_backend_buffer_get_type(t->buffer);
    const char * buffer_name = ggml_backend_buffer_name(t->buffer);
    const char * buft_name = buft != nullptr ? ggml_backend_buft_name(buft) : nullptr;

    return llama_hexagon_name_contains(buffer_name, "HTP") ||
           llama_hexagon_name_contains(buffer_name, "Hexagon") ||
           llama_hexagon_name_contains(buft_name, "HTP") ||
           llama_hexagon_name_contains(buft_name, "Hexagon");
}

static bool llama_hexagon_tensor_is_repack_resident(const ggml_tensor * t) {
    if (!llama_hexagon_tensor_is_resident(t)) {
        return false;
    }

    const char * hostbuf = std::getenv("GGML_HEXAGON_HOSTBUF");
    if (hostbuf != nullptr && std::atoi(hostbuf) == 0) {
        return true;
    }
    ...
    return llama_hexagon_name_contains(buffer_name, "REPACK") ||
           llama_hexagon_name_contains(buffer_name, "repack") ||
           llama_hexagon_name_contains(buft_name, "REPACK") ||
           llama_hexagon_name_contains(buft_name, "repack");
}

static bool llama_hexagon_can_build_fused_lora(
        const ggml_tensor * w,
        const ggml_tensor * cur,
        const llama_adapter_lora_weight * lw) {
    if (lw == nullptr || lw->a == nullptr || lw->b == nullptr) {
        return false;
    }

    const ggml_tensor * a = lw->a;
    const ggml_tensor * b = lw->b;

    if (!llama_hexagon_fused_lora_weight_name_supported(w)) {
        return false;
    }

    if ((w->type != GGML_TYPE_F16 && w->type != GGML_TYPE_Q4_0) ||
    ...
    if (w->type == GGML_TYPE_Q4_0 && (k % 256) != 0) {
        return false;
    }
...
static bool llama_hexagon_can_place_fused_lora(
        const ggml_tensor * w,
        const llama_adapter_lora_weight * lw,
        bool antithetic_rm) {
    if (lw == nullptr || !llama_hexagon_tensor_is_resident(w) || !llama_hexagon_tensor_is_resident(lw->a)) {
        return false;
    }

    if (w->type == GGML_TYPE_Q4_0 && !llama_hexagon_tensor_is_repack_resident(w)) {
        return false;
    }

    if (antithetic_rm) {
        return llama_hexagon_tensor_is_resident(lw->b_plus_rm) &&
               llama_hexagon_tensor_is_resident(lw->b_minus_rm);
    }

    return llama_hexagon_tensor_is_resident(lw->b);
}

static bool llama_antithetic_has_paired_rows(const ggml_tensor * cur) {
    return cur != nullptr &&
           cur->ne[2] == 1 &&
           cur->ne[3] == 1 &&
           cur->ne[1] > 1 &&
...
static ggml_tensor * llama_build_runtime_antithetic_lora(
        ggml_context * ctx,
        ggml_tensor * cur,
        const llama_adapter_lora_weight * lw,
        float scale) {
    if (lw == nullptr || lw->a == nullptr || lw->b_plus == nullptr || lw->b_minus == nullptr) {
        throw std::runtime_error("--antithetic true requires CPU LoRA B+/B- tensors for runtime fallback");
    }
    if (!llama_antithetic_has_paired_rows(cur)) {
        throw std::runtime_error("--antithetic true CPU runtime fallback requires an even 2D paired batch");
    }

    const int64_t positive_rows = cur->ne[1] / 2;
    const int64_t negative_rows = cur->ne[1] - positive_rows;
    const size_t row_stride = cur->nb[1];

    ggml_tensor * cur_plus = ggml_view_2d(ctx, cur, cur->ne[0], positive_rows, row_stride, 0);
    ggml_tensor * cur_minus = ggml_view_2d(ctx, cur, cur->ne[0], negative_rows, row_stride, (size_t) positive_rows * row_stride);

    ggml_tensor * delta_plus = ggml_mul_mat(ctx, lw->b_plus, ggml_mul_mat(ctx, lw->a, cur_plus));
    ggml_tensor * delta_minus = ggml_mul_mat(ctx, lw->b_minus, ggml_mul_mat(ctx, lw->a, cur_minus));
    ggml_tensor * delta = ggml_concat(ctx, delta_plus, delta_minus, 1);

    return ggml_scale(ctx, delta, scale);
}
...
ggml_tensor * llm_graph_context::build_lora_mm(
    ...
    const bool antithetic_lora = llama_hexagon_antithetic_lora_enabled();

    if (llama_hexagon_fused_lora_enabled() && loras->size() == 1) {
        const auto & lora = *loras->begin();
        llama_adapter_lora_weight * lw = lora.first->get_weight(w);
        const bool antithetic_rm =
            antithetic_lora &&
            lw != nullptr &&
            lw->b_plus_rm != nullptr &&
            lw->b_minus_rm != nullptr;
        if (llama_hexagon_can_build_fused_lora(w, cur, lw) &&
                llama_hexagon_can_place_fused_lora(w, lw, antithetic_rm)) {
        const float adapter_scale = lora.second;
        const float scale = lw->get_scale(lora.first->alpha, adapter_scale);

            ggml_tensor * res = nullptr;
            if (antithetic_lora && lw->b_plus_rm != nullptr && lw->b_minus_rm != nullptr &&
                    lw->b_plus_rm->type  == GGML_TYPE_F32 &&
                    lw->b_minus_rm->type == GGML_TYPE_F32 &&
                    lw->b_plus_rm->ne[0] == lw->b->ne[1] &&
                    lw->b_plus_rm->ne[1] == lw->b->ne[0] &&
                    lw->b_plus_rm->ne[2] == lw->b->ne[2] &&
                    lw->b_plus_rm->ne[3] == lw->b->ne[3] &&
                    lw->b_minus_rm->ne[0] == lw->b_plus_rm->ne[0] &&
                    lw->b_minus_rm->ne[1] == lw->b_plus_rm->ne[1] &&
                    lw->b_minus_rm->ne[2] == lw->b_plus_rm->ne[2] &&
                    lw->b_minus_rm->ne[3] == lw->b_plus_rm->ne[3] &&
                    (lw->a->ne[1] % 8) == 0 && lw->a->ne[1] <= 32) {
                const int64_t n_rows = cur->ne[1];
                if ((n_rows % 2) == 0) {
                    res = ggml_lora_mul_mat_antithetic_rm(
                            ctx0, w, cur, lw->a, lw->b_plus_rm, lw->b_minus_rm, scale,
                            (int32_t) (n_rows / 2));
                }
            } else if (antithetic_lora && lw->b_plus != nullptr && lw->b_minus != nullptr &&
                    llama_hexagon_tensor_is_resident(lw->b_plus) &&
                    llama_hexagon_tensor_is_resident(lw->b_minus) &&
                    lw->b_plus->type  == GGML_TYPE_F16 &&
                    lw->b_minus->type == GGML_TYPE_F16 &&
                    lw->b_plus->ne[0] == lw->b->ne[0] &&
                    lw->b_plus->ne[1] == lw->b->ne[1] &&
                    lw->b_plus->ne[2] == lw->b->ne[2] &&
                    lw->b_plus->ne[3] == lw->b->ne[3] &&
                    lw->b_minus->ne[0] == lw->b->ne[0] &&
                    lw->b_minus->ne[1] == lw->b->ne[1] &&
                    lw->b_minus->ne[2] == lw->b->ne[2] &&
                    lw->b_minus->ne[3] == lw->b->ne[3] &&
                    (lw->a->ne[1] % 8) == 0 && lw->a->ne[1] <= 32) {
                const int64_t n_rows = cur->ne[1];
                if ((n_rows % 2) == 0) {
                    res = ggml_lora_mul_mat_antithetic(
                            ctx0, w, cur, lw->a, lw->b_plus, lw->b_minus, scale,
                            (int32_t) (n_rows / 2));
                }
            }
            if (res == nullptr) {
                if (antithetic_lora && llama_antithetic_has_paired_rows(cur)) {
                    throw std::runtime_error("--antithetic true requires usable LoRA B+/B- tensors for fused HTP path");
                }
                res = ggml_lora_mul_mat(ctx0, w, cur, lw->a, lw->b, scale);
            }
            if (w_s) {
    ...
    const bool debug_lora_branch = llama_lora_debug_branch_enabled();
    ...
    for (const auto & lora : *loras) {
        ...
        if (antithetic_lora && llama_antithetic_has_paired_rows(cur)) {
            ggml_tensor * ab_cur = llama_build_runtime_antithetic_lora(ctx0, cur, lw, scale);
            res = ggml_add(ctx0, res, ab_cur);
        } else if (debug_lora_branch) {
            const std::string w_name = ggml_get_name(w);
            cb(res, (std::string("lora_base:") + w_name).c_str(), -1);
            ggml_tensor * xA_raw = ggml_mul_mat(ctx0, lw->a, cur);
            cb(xA_raw, (std::string("lora_xA_raw:") + w_name).c_str(), -1);
            ggml_tensor * xA_scaled = ggml_scale(ctx0, xA_raw, scale);
            cb(xA_scaled, (std::string("lora_xA_scaled:") + w_name).c_str(), -1);
            ggml_tensor * xAB_scaled = ggml_mul_mat(ctx0, lw->b, xA_scaled);
            cb(xAB_scaled, (std::string("lora_xAB_scaled:") + w_name).c_str(), -1);
            res = ggml_add(ctx0, res, xAB_scaled);
            cb(res, (std::string("lora_add_out:") + w_name).c_str(), -1);
        } else {
