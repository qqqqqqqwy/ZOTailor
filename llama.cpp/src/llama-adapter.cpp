...
static void llama_adapter_lora_init_impl(llama_model & model, const char * path_lora, llama_adapter_lora & adapter) {
    ...
    const char * antithetic_env = std::getenv("GGML_HEXAGON_ANTITHETIC_LORA");
    const bool alloc_antithetic_b =
        antithetic_env != nullptr &&
        std::strcmp(antithetic_env, "0") != 0 &&
        std::strcmp(antithetic_env, "false") != 0 &&
        std::strcmp(antithetic_env, "off") != 0;

    // The GGUF contains one .lora_a and one .lora_b per adapted weight. In the
    // antithetic path we materialize four extra low-rank B tensors:
    // b_plus/b_minus for the legacy paired path and b_plus_rm/b_minus_rm for
    // the CPU-prepacked rank-major F32 path.
    const size_t ctx_tensor_slots = (size_t) n_tensors * (alloc_antithetic_b ? 3u : 2u);
    ...
    for (auto & it : ab_map) {
        ...
        ggml_tensor * tensor_b_plus  = alloc_antithetic_b ? ggml_dup_tensor(dev_ctx, w.b) : nullptr;
        ggml_tensor * tensor_b_minus = alloc_antithetic_b ? ggml_dup_tensor(dev_ctx, w.b) : nullptr;
        ggml_tensor * tensor_b_plus_rm  = alloc_antithetic_b ? ggml_new_tensor_2d(dev_ctx, GGML_TYPE_F32, w.b->ne[1], w.b->ne[0]) : nullptr;
        ggml_tensor * tensor_b_minus_rm = alloc_antithetic_b ? ggml_new_tensor_2d(dev_ctx, GGML_TYPE_F32, w.b->ne[1], w.b->ne[0]) : nullptr;
        ggml_set_name(tensor_a, w.a->name);
        ggml_set_name(tensor_b, w.b->name);

        if (tensor_b_plus && tensor_b_minus && tensor_b_plus_rm && tensor_b_minus_rm) {
            std::string plus_name = w.b->name;
            std::string minus_name = w.b->name;
            std::string plus_rm_name = w.b->name;
            std::string minus_rm_name = w.b->name;
            replace_all(plus_name, ".lora_b", ".lora_b_plus");
            replace_all(minus_name, ".lora_b", ".lora_b_minus");
            replace_all(plus_rm_name, ".lora_b", ".lora_b_plus_rm");
            replace_all(minus_rm_name, ".lora_b", ".lora_b_minus_rm");
            ggml_set_name(tensor_b_plus, plus_name.c_str());
            ggml_set_name(tensor_b_minus, minus_name.c_str());
            ggml_set_name(tensor_b_plus_rm, plus_rm_name.c_str());
            ggml_set_name(tensor_b_minus_rm, minus_rm_name.c_str());
        }

        adapter.ab_map[name] = llama_adapter_lora_weight(tensor_a, tensor_b, tensor_b_plus, tensor_b_minus, tensor_b_plus_rm, tensor_b_minus_rm);
    }
    ...
    // set tensor data
    {
        ...
        for (auto & it : adapter.ab_map) {
            ...
            if (dev.b_plus && dev.b_minus) {
                read_buf.assign(ggml_nbytes(dev.b_plus), 0);
                ggml_backend_tensor_set(dev.b_plus, read_buf.data(), 0, read_buf.size());
                ggml_backend_tensor_set(dev.b_minus, read_buf.data(), 0, read_buf.size());
            }
            if (dev.b_plus_rm && dev.b_minus_rm) {
                read_buf.assign(ggml_nbytes(dev.b_plus_rm), 0);
                ggml_backend_tensor_set(dev.b_plus_rm, read_buf.data(), 0, read_buf.size());
                ggml_backend_tensor_set(dev.b_minus_rm, read_buf.data(), 0, read_buf.size());
            }
        }
    }
