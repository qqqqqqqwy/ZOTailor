...
try:
    ...
    from .lora_fa_tinyllama import (
        TinyLlamaLoraFAForPrefillLoss,
        initialize_lora_a_kaiming_uniform,
        load_hf_state_dict_into_lora_fa_model,
    )
except ImportError:
    from llama import ModelArgs  # type: ignore[no-redef]
    from lora_fa_tinyllama import (  # type: ignore[no-redef]
        TinyLlamaLoraFAForPrefillLoss,
        initialize_lora_a_kaiming_uniform,
        load_hf_state_dict_into_lora_fa_model,
    )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Export TinyLlama LoRA-FA prefill loss model to ExecuTorch."
    )
    parser.add_argument(
        "--hf_model",
    ...
    parser.add_argument(
        "--config",
        default="mytest/tinyllama_config.json",
        help="TinyLlama config JSON compatible with mytest/llama.py.",
    )
    ...
    parser.add_argument(
        "--pte_name",
        default="tinyllama_lora_fa.pte",
        help="Output ExecuTorch program filename.",
    )
    parser.add_argument(
        "--weights_name",
        default="tinyllama_lora_fa_weights.ptd",
...
def save_lora_layout(
    ...
    layout_path = output_dir / "tinyllama_lora_fa_layout.json"
