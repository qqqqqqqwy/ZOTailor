    ...
    def from_hf_config(
        ...
        *,
        ...
        if config.get("model_type") != "llama":
            raise ValueError(
                f"Expected HF config model_type='llama', got {config.get('model_type')!r}"
            )
        max_position_embeddings = int(config.get("max_position_embeddings", min_seq_len))
        return cls(
            ...
            norm_eps=float(config.get("rms_norm_eps", 1.0e-5)),
...
def _llama_inv_freq(
    ...
    *,
    ...
    if rope_type is None:
        return inv_freq
    if rope_type != "llama3":
        raise ValueError(f"Unsupported Llama RoPE scaling type: {rope_type!r}")
    ...
    is_medium_freq = torch.logical_and(
        wavelen <= low_freq_wavelen,
...
class LoraFALinear(nn.Module):
    def __init__(
        ...
        *,
        ...
        super().__init__()
        if bias:
            raise ValueError("Llama linear layers are expected to be bias-free")
...
class LlamaLoraFAForPrefillLoss(nn.Module):
    def __init__(
        self,
        params: LlamaModelArgs,
        *,
        batch_size: int = 4,
        seq_len: int = 32,
        lora_rank: int = 8,
        ...
        rope_cos, rope_sin = build_llama_rope_cos_sin(
