...
try:
    from .lora_fa_qwen import (
        QwenLoraFAForPrefillLoss,
        QwenModelArgs,
        initialize_lora_a_kaiming_uniform,
        load_hf_state_dict_into_lora_fa_model,
    )
except ImportError:
    from lora_fa_qwen import (  # type: ignore[no-redef]
        QwenLoraFAForPrefillLoss,
        QwenModelArgs,
        initialize_lora_a_kaiming_uniform,
        load_hf_state_dict_into_lora_fa_model,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Export Qwen-family LoRA-FA prefill loss model to ExecuTorch."
    )
    parser.add_argument(
        "--hf_model",
        default="/home/fuyongjian/qwy/models/Qwen3.5-2B",
        help="Hugging Face model id or local model directory.",
    )
    parser.add_argument("--output_dir", default="mytest/out")
    parser.add_argument(
        "--log_path",
        default="mytest/out/qwen35_2b_export_log.txt",
        help="Write Python export/smoke logs to this text file.",
    )
    parser.add_argument(
        "--pte_name",
        default="qwen35_2b_lora_fa.pte",
        help="Output ExecuTorch program filename.",
    )
    parser.add_argument(
        "--weights_name",
        default="qwen35_2b_lora_fa_weights.ptd",
    ...
    parser.add_argument(
        "--trust_remote_code",
        action="store_true",
        help="Pass trust_remote_code=True to Hugging Face AutoConfig/AutoModel.",
    )
    parser.add_argument(
        "--skip_eager_smoke",
        action="store_true",
...
def load_model_args(
    ...
    *,
...
) -> Tuple[QwenModelArgs, Dict[str, object]]:
    ...
    model_args = QwenModelArgs.from_hf_config(
        hf_config_dict,
        min_seq_len=seq_len,
    )
...
def save_lora_layout(
    ...
    layout_path = output_dir / "qwen35_2b_lora_fa_layout.json"
...
def main() -> None:
    ...
    logging.info(
        ...
        "rope_scaling=%s tie_word_embeddings=%s attention_bias=%s mlp_bias=%s "
        "use_qk_norm=%s",
